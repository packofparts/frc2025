...
public class Camera {
    ...
    public Camera(CameraConfig config) {
        ...
        if (config.pipelineType == PipelineType.APRIL_TAG) {
    ...
    public Optional<PhotonTrackedTarget> getDetectedObject() {
        Optional<PhotonTrackedTarget> ret = Optional.empty();
        if (config.pipelineType == PipelineType.OBJECT_DETECTOR) {
            PhotonTrackedTarget target = null;
            for (PhotonPipelineResult res : camera.getAllUnreadResults()) {
                if (target == null || res.getBestTarget().area > target.area) {
                    target = res.getBestTarget();
                }
            }
            if (target != null) {
                ret = Optional.of(target);
            }
        }
        return ret;
    }
