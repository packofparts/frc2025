...
public abstract class VisionBaseSwerve extends BaseSwerve {
    ...
    public Optional<Double> getRotationNeededForObjectDetection() {
        Optional<PhotonTrackedTarget> target = Optional.empty();
        Transform3d offset = null;
        for (Camera camera : cameras) {
            target = camera.getDetectedObject();
            if (target.isPresent()) {
                offset = camera.config.cameraToRobot;
                break;
            }
        }
        if (offset == null) {
        ...
        double x = Math.tan(Math.toRadians(90 + target.get().getPitch())) * offset.getZ();
        double y = x / Math.tan(Math.toRadians(target.get().getYaw()));
        return Optional.of(-(y - offset.getY()));
    }
